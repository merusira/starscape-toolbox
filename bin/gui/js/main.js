...
const { TeraToolboxMUI: TeraStarscapeMUI, LanguageNames } = require("tera-toolbox-mui");
const Themes = ["atlas", "toolbox", "dark", "light", "retro", "starscape"];
const fs = require("fs");

// Banner image for Starscape
const bannerImage = "assets/banner-starscape.png";
...
jQuery(($) => {
	...
	// Preload banner image when the application starts
	preloadBannerImage();
	...
	function setLanguage(language) {
		...
		mui = new TeraStarscapeMUI(language);
		$("*").each(function () {
...
function onSettingsChanged(newSettings) {
	...
	// Always set patch version to Starscape
	Settings.patchVersion = "100.02 Starscape";
	
	// Set banner image
	$("#patchBanner").attr("src", bannerImage);
}
	...
	function log(msg, type) {
		...
		contents.append($("<div/>", { "class": type || "log" }).html(`${timeStr}${msg}`));
	...
	ipcRenderer.on("set mods", (_, modInfos) => {
		...
		modInfos.forEach(modInfo => {
			...
			// Modified line 363 and 365 to always have a space even with html code present.
			const summaryId = `modsummary-${escapedName}`;
			$("#modulesList").append(`
				<div  class="${headerClasses} noselect">
					<div class="mod-always-visible-controls">
						${(!modInfo.isCoreModule && modInfo.compatibility === "compatible") ? `<div data-microtip-position="bottom-left" aria-label="${mui.get("gui/tooltip/toggleModAutoupdate")}" role="tooltip" class="mod-action-button ${modInfo.disableAutoUpdate ? 'mod-action-button-red' : ''}" id="${updateId}"><i class="mdi ${autoUpdateClass}"></i></div>` : ""}
						${(!modInfo.isCoreModule && modInfo.compatibility === "compatible") ? `<div data-microtip-position="bottom-left" aria-label="${mui.get("gui/tooltip/toggleMod")}" role="tooltip" class="mod-action-button ${modInfo.disabled ? 'mod-action-button-red' : ''}" id="${enabledId}"><i class="mdi ${enabledClass}"></i></div>` : ""}
					</div>
					<details id="${headerId}" ${expandedModsSummary[modInfo.name] ? "open" : ""}>
						<summary id="${summaryId}">
								${modInfo.drmKey ? "<span class=\"mdi mdi-currency-usd\"></span>" : ""} ${displayName(modInfo)} ${modInfo.version ? `<span style="margin-left: 5px;">(${modInfo.version})</span>` : ""}
								<div class="spacer"></div>
								${modInfo.author ? `<span class="mod-author">by <span style="margin-left: 5px;">${modInfo.author}</span></span>` : ""} </summary>
	...
	function rebuildInstallableModsList() {
		...
		InstallableModInfos.filter(modInfo => matchesInstallableModFilter(modInfo)).forEach(modInfo => {
			...
			// Modified line 473 and 475 to always have a space even with html code present.
			$("#installableModulesList").append(`
				<div id="${headerId}" class="mod-info noselect">
					<details open>
						<summary>
							${displayName(modInfo)} ${modInfo.version ? `<span style="margin-left: 5px;">(${modInfo.version})</span>` : ""}
							<div class="spacer"></div>
							${modInfo.author ? `by <span style="margin-left: 5px;">${modInfo.author}</span>` : ""}
						</summary>
						<p>${modInfo.description ? modInfo.description : " "}</p>
						<div class="mod-info-controls">
