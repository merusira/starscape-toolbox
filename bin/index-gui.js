...
const SupportURL = 'https://github.com/merusira/starscape-toolbox/issues';
...
async function updateSelf() {
    ...
    updater.on('check_fail', (serverIndex, e) => {
        // errors.push(`TeraStarscape was unable to check for updates on server ${serverIndex}!\n${e}`);
    ...
    updater.on('install_error', (relpath, e) => {
        ...
        if (relpath.startsWith('node_modules/tera-client-interface/scanner/')) {
            ...
            console.log('[update] - Please whitelist TeraStarscape in your anti-virus!');
        ...
        if (relpath.startsWith('node_modules/tera-client-interface/scanner/'))
            errors.push(`Unable to install "${relpath}"!\n${e}\n\nYour anti-virus software most likely falsely detected TeraStarscape to be a virus.\nPlease whitelist it!`);
        else if (relpath === 'node_modules/tera-client-interface/tera-client-interface.dll')
    ...
    if (noselfupdate) {
        ...
        updateSelf().then(errors => {
            if (errors && errors.length > 0) {
                let errmsg = `TeraStarscape was unable to update itself. If the problem persists, ask here ${SupportURL} for help!\n\nThe full error message is:\n\n------------------------------\n`;
                errmsg += errors.join('\n------------------------------\n');
                ...
                if (updateRequired()) {
                    ...
                    update().then(() => {
                        ...
                        dialogOnly({
                            ...
                            message: `TeraStarscape was unable to update Electron. If the problem persists, ask here ${SupportURL} for help!\n\nThe full error message is:\n${e}\n\nThe program will continue running, but some features may not work correctly.`
                        });
            ...
            dialogOnly({
                type: 'error',
                title: 'Self-update error!',
                message: `TeraStarscape was unable to update itself. If the problem persists, ask here ${SupportURL} for help!\n\nThe full error message is:\n${e}\n\nThe program will continue running, but some features may not work correctly.`
            });
